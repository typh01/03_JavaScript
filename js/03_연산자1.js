...
/* Number(input1.value)가 계속 반복되는데 
   ...
   왜? 전역 변수는 페이지 로딩 시점에 해석되서 값이 결정된다!
    -> input에 작성된 값이 없어서 Number(inpur1.value) == 0

    -> (해결 방법)
       값을 입력한 후 함수를 호출해서
       함수가 호출되는 시점의
       입력된 값을 읽어 올 수 있게 만든다!!
*/
...
function plusFn(){
  ...
  // inpute1.value : #input1 요소에 작성된 값('string') 얻어오기
...
/** 나누기 함수 */
function divFn(){
  console.log("두 수의 나누기 : ", Number(input1.value) / Number(input2.value));
  calcResult.innerText = Number(input1.value) / Number(input2.value);
}

/** 나머지 함수 */
function modFn(){
  console.log("두 수의 나머지 : ", Number(input1.value) % Number(input2.value));
  calcResult.innerText = Number(input1.value) % Number(input2.value);
}
...
/** 첫번째 값 X2 */
function doubleFn(){
  console.log("첫번째 값 X2 : ", Number(input3.value) * 2);
...
function allOperationFn(){
  ...
  // 요소의 영역 margin -> border -> padding -> contemnt
/* 
  요소.innerText = 문자열
    -> <span>내용</span> 내용 부분에 "문자열" 대입(출력)
    -> HTML 코드 해석 X 
  ex) calcResult2.innerText = "<i>기울어짐</i>"
*/
...
/* 
  요소.innerHTML = 문자열
    -> <span>내용</span> 내용 부분에 "문자열" 대입(출력)
    -> HTML 코드 해석 O 
  ex) calcResult2.innerHTML = "<i>기울어짐</i>"
*/
