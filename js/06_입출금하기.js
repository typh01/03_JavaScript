...
function deposit(){
  ...
  // 입력된 금액을 얻어와 number타입으로 변환 후 balance 변수에 누적
  balance += Number(amount.value);
  // balance = balance + Number(amount.value);

  output.innerText = balance;// 화면에 누적된 잔액 출력력
  amount.value = ""; // 입력된 금액 삭제
}

/* 내가 푼 것
function deposit(){
  const inputNum = Number(amount.value);
  balance = balance + inputNum

  if(amount === null || amount.value.length === 0){
    alert("금액을 입력해주세요")
  }
  if(amount <=0){
...
function withdrawal(){
  ...
  if(balance < inputNum){
    ...
    balance = balance - inputNum
    alert(`${inputNum}원이 출금 되었습니다. 남은 잔액 : ${balance}원`)
    output.innerText = balance;
    amount.value = "";
  }
} */